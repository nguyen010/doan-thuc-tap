...
import { LayoutDashboard, Calendar, History, ShieldCheck, Users, LogOut, Menu, Home } from 'lucide-react';
...
export function Sidebar({ onClose }: { onClose?: () => void }) {
  ...
  const menuItems = [
    ...
    { name: t('checkin'), icon: ShieldCheck, path: '/checkin', roles: ['EVENT_MANAGER', 'ADMIN'] },
  ...
  return (
    ...
    >
      <div className="p-6 h-20 flex items-center justify-between shrink-0 border-b border-slate-50">
        <Logo collapsed={isCollapsed} variant="dark" className="scale-75 md:scale-90 origin-left" />
        <Button
          variant="ghost"
          size="icon"
      ...
      {user && (
        <div className="p-4 shrink-0">
          <div className={cn(
            "flex items-center gap-3 p-3 bg-slate-900 border border-slate-800 rounded-3xl transition-all shadow-xl shadow-slate-200/50 relative overflow-hidden group",
